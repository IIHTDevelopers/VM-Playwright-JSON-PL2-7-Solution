...
async deleteEvents(editEventName: string) {
    ...
    // await this.saveBtn.click();
    // await this.page.locator("//div[@role='row']/div[2]")
    //     .locator(`//div[text()='${editEventName}']`)
...
async inactivateEvents(togEventName: string) {
    ...
    // await this.saveBtn.click();
    // await this.page.locator(`//div[contains(@class, 'oxd-table-card') and contains(., '${togEventName}')]/div/div[3]`).waitFor({ state: 'visible' });
