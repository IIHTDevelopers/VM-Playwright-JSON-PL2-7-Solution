 ...
 */
  ...
  const qualifications = await  page.locator("//div[@class='oxd-table']").nth(0).locator("//div/div/div/div[2]").allInnerTexts();
 ...
 */
  ...
  test("TS-2 Verify New 'Qualification' added could be edited from the record of user ", async ({ page }) => {
   ...
   const editlist= await page.locator("//div[@class='oxd-table']").nth(0).locator("//div/div/div/div[2]").allInnerTexts();
 ...
 */
  ...
  test("TS-3 Verify the qualification could be deleted", async ({
    ...
    const delList= await page.locator("//div[@class='oxd-table']").nth(0).locator("//div/div/div/div[2]").allInnerTexts();
 ...
 */
  ...
  test("TS-5 Verify New 'Education' could be added to the record of user", async ({
   ...
   const GPAList =  await page.locator("//div[contains(@class, 'orangehrm-container')]").nth(1).locator("//div[@role='row']/div[4]").allInnerTexts();
 ...
 */
test("TC-6 Verify Event tab URL in Claim configuration", async ({page}) => {
    await ClaimPage.clickClaimButton();
    const url = page.url();
    expect(url).toContain(Baseurl + "/web/index.php/claim/viewEvents");
});
...
test("TS-7 Verify Event creation in Claim configuration", async ({ page }) => {
  ...
  await ClaimPage.addevents(eventName);
  const list = await page.locator("//div[@role='row']/div[2]").allInnerTexts();
...
test("TS-8 Verify Event Deletion in Claim Configuration", async ({ page }) => {
  ...
  await ClaimPage.deleteEvents(eventName);
  const List = await page.locator("//div[@role='row']/div[2]").allInnerTexts();
...
test("TS-9 Event Inactive in Claim Configuration", async ({ page }) => {
  ...
  await ClaimPage.inactivateEvents(eventName);
  const status = await page.locator(`//div[contains(@class, 'oxd-table-card') and contains(., '${eventName}')]/div/div[3]`).innerText();
...
test("TS-10 Verify that a new Expense Type can be added", async ({ page }) => {
  ...
  await ClaimPage.addExpense(expenseName);
  const list = await page.locator("//div[@role='row']/div[2]").allInnerTexts();
  expect(list && list.length > 0).toBeTruthy(); // Ensure the list is not empty
...
test("TS-11 Verify the 'Skills' could be deleted from the record", async ({ page }) => {
  ...
  await myinfoPage.deleteskills(year);
  const skillList = await page.locator("//div[contains(@class, 'orangehrm-container')]").nth(2).locator("//div[@role='row']/div[3]/div").allInnerTexts(); 
...
test("TS-12 Verify the 'Skills' could be selected delete functionality", async ({ page }) => {
  const year = Math.floor(Math.random() * 100) - 1;
  await myinfoPage.selectDeleteSkills(year); 
  const skillList = await page.locator("//div[contains(@class, 'orangehrm-container')]").nth(2).locator("//div[@role='row']/div[3]/div").allInnerTexts();
  expect(skillList.length).toBeGreaterThan(0);
  expect(skillList).not.toContain(year);
});
...
test("TS-13 Verify that an Expense Type can be edited", async ({ page }) => {
  ...
  await ClaimPage.editExpense(EditexpenseName);
  const list = await  page.locator("//div[@role='row']/div[2]").allInnerTexts();
  expect(list && list.length > 0).toBeTruthy(); // Ensure the list is not empty
...
/**
 * Test Case: TS-14 Verify that an Expense Type can be deleted
 *
 * Objective:
 ...
 */
  test("TS-14 Verify the Immigrant assign immigration record Delete selected Functionality", async ({ page }) => {
    ...
    await ClaimPage.deleteExpense(EditexpenseName);
    const expenseList = await page.locator("//div[@role='row']/div[2]").allInnerTexts();
 ...
 */
test("TC-15 Verify that multiple Expense Types can be selected and deleted", async ({page}) => {   
  const EditexpenseName = `delExp_${Math.floor(Math.random() * 1000)}`;                         // Step 1: Unique comment
  await ClaimPage.multipleExpenseDel(EditexpenseName);
  const list = await page.locator("//div[@role='row']/div[2]").allInnerTexts();
  expect(list && list.length > 0).toBeTruthy(); // Ensure the list is not empty
